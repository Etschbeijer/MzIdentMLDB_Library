...
module XMLParsing =
    ...
    let convertToEntityCVParam (dbContext:MzIdentMLContext) (mzIdentMLXML:SchemePeptideShaker.CvParam) =
    ...
    let convertToEntityModificationParam (dbContext:MzIdentMLContext) (mzIdentMLXML:SchemePeptideShaker.CvParam) =
        let init = ModificationParamHandler.init(mzIdentMLXML.Name, ModificationParamHandler.findTermByID dbContext (mzIdentMLXML.Accession))
        let addValue = setOption ModificationParamHandler.addValue init mzIdentMLXML.Value
        let addUnit = ModificationParamHandler.addUnit addValue (match mzIdentMLXML.UnitAccession with
                                                                 |Some x -> ModificationParamHandler.findTermByID dbContext (mzIdentMLXML.UnitAccession.Value) 
                                                                 |None -> Unchecked.defaultof<Term>
                                                                )
        let addUnitName = setOption ModificationParamHandler.addUnitName addUnit mzIdentMLXML.UnitName
        addUnitName

    let convertToEntityTranslationTableParam (dbContext:MzIdentMLContext) (mzIdentMLXML:SchemePeptideShaker.CvParam) =
        let init = TranslationTableParamHandler.init(mzIdentMLXML.Name, TranslationTableParamHandler.findTermByID dbContext (mzIdentMLXML.Accession))
        let addValue = setOption TranslationTableParamHandler.addValue init mzIdentMLXML.Value
        let addUnit = TranslationTableParamHandler.addUnit addValue (match mzIdentMLXML.UnitAccession with
                                                                     |Some x -> TranslationTableParamHandler.findTermByID dbContext (mzIdentMLXML.UnitAccession.Value) 
                                                                     |None -> Unchecked.defaultof<Term>
                                                                    )
        let addUnitName = setOption TranslationTableParamHandler.addUnitName addUnit mzIdentMLXML.UnitName
        addUnitName

    let convertToEntityMeasureParam (dbContext:MzIdentMLContext) (mzIdentMLXML:SchemePeptideShaker.CvParam) =
        let init = MeasureParamHandler.init(mzIdentMLXML.Name, MeasureParamHandler.findTermByID dbContext (mzIdentMLXML.Accession))
        let addValue = setOption MeasureParamHandler.addValue init mzIdentMLXML.Value
        let addUnit = MeasureParamHandler.addUnit addValue (match mzIdentMLXML.UnitAccession with
                                                       |Some x -> MeasureParamHandler.findTermByID dbContext (mzIdentMLXML.UnitAccession.Value) 
                                                       |None -> Unchecked.defaultof<Term>
                                                      )
        let addUnitName = setOption MeasureParamHandler.addUnitName addUnit mzIdentMLXML.UnitName
        addUnitName

    let convertToEntitySearchModificationParam (dbContext:MzIdentMLContext) (mzIdentMLXML:SchemePeptideShaker.CvParam) =
        let init = SearchModificationParamHandler.init(mzIdentMLXML.Name, MeasureParamHandler.findTermByID dbContext (mzIdentMLXML.Accession))
        let addValue = setOption SearchModificationParamHandler.addValue init mzIdentMLXML.Value
        let addUnit = SearchModificationParamHandler.addUnit addValue (match mzIdentMLXML.UnitAccession with
                                                                       |Some x -> SearchModificationParamHandler.findTermByID dbContext (mzIdentMLXML.UnitAccession.Value) 
                                                                       |None -> Unchecked.defaultof<Term>
                                                                      )
        let addUnitName = setOption SearchModificationParamHandler.addUnitName addUnit mzIdentMLXML.UnitName
        addUnitName

    let convertToEntitySpecificityRuleParam (dbContext:MzIdentMLContext) (mzIdentMLXML:SchemePeptideShaker.CvParam) =
        let init = SpecificityRuleParamHandler.init(mzIdentMLXML.Name, SpecificityRuleParamHandler.findTermByID dbContext (mzIdentMLXML.Accession))
        let addValue = setOption SpecificityRuleParamHandler.addValue init mzIdentMLXML.Value
        let addUnit = SpecificityRuleParamHandler.addUnit addValue (match mzIdentMLXML.UnitAccession with
                                                                    |Some x -> SpecificityRuleParamHandler.findTermByID dbContext (mzIdentMLXML.UnitAccession.Value) 
                                                                    |None -> Unchecked.defaultof<Term>
                                                                    )
        let addUnitName = setOption SpecificityRuleParamHandler.addUnitName addUnit mzIdentMLXML.UnitName
        addUnitName

    let convertToEntityFragmentToleranceParam (dbContext:MzIdentMLContext) (mzIdentMLXML:SchemePeptideShaker.CvParam) =
        let init = FragmentToleranceParamHandler.init(mzIdentMLXML.Name, FragmentToleranceParamHandler.findTermByID dbContext (mzIdentMLXML.Accession))
        let addValue = setOption FragmentToleranceParamHandler.addValue init mzIdentMLXML.Value
        let addUnit = FragmentToleranceParamHandler.addUnit addValue (match mzIdentMLXML.UnitAccession with
                                                                      |Some x -> FragmentToleranceParamHandler.findTermByID dbContext (mzIdentMLXML.UnitAccession.Value) 
                                                                      |None -> Unchecked.defaultof<Term>
                                                                     )
        let addUnitName = setOption FragmentToleranceParamHandler.addUnitName addUnit mzIdentMLXML.UnitName
        addUnitName

    let convertToEntityParentToleranceParam (dbContext:MzIdentMLContext) (mzIdentMLXML:SchemePeptideShaker.CvParam) =
        let init = ParentToleranceParamHandler.init(mzIdentMLXML.Name, ParentToleranceParamHandler.findTermByID dbContext (mzIdentMLXML.Accession))
        let addValue = setOption ParentToleranceParamHandler.addValue init mzIdentMLXML.Value
        let addUnit = ParentToleranceParamHandler.addUnit addValue (match mzIdentMLXML.UnitAccession with
                                                                    |Some x -> ParentToleranceParamHandler.findTermByID dbContext (mzIdentMLXML.UnitAccession.Value) 
                                                                    |None -> Unchecked.defaultof<Term>
                                                                    )
        let addUnitName = setOption ParentToleranceParamHandler.addUnitName addUnit mzIdentMLXML.UnitName
        addUnitName

    let convertToEntitySearchDatabaseParam (dbContext:MzIdentMLContext) (mzIdentMLXML:SchemePeptideShaker.CvParam) =
        let init = SearchDatabaseParamHandler.init(mzIdentMLXML.Name, SearchDatabaseParamHandler.findTermByID dbContext (mzIdentMLXML.Accession))
        let addValue = setOption SearchDatabaseParamHandler.addValue init mzIdentMLXML.Value
        let addUnit = SearchDatabaseParamHandler.addUnit addValue (match mzIdentMLXML.UnitAccession with
                                                                   |Some x -> SearchDatabaseParamHandler.findTermByID dbContext (mzIdentMLXML.UnitAccession.Value) 
                                                                   |None -> Unchecked.defaultof<Term>
                                                                  )
        let addUnitName = setOption SearchDatabaseParamHandler.addUnitName addUnit mzIdentMLXML.UnitName
        addUnitName
...
//Stup because there is a problem with the xmlFile with the values of the FragmentArray/////////////////
        ...
        let enzymeWithEnzymeNames = 
            EnzymeHandler.addEnzymeNames
                ...
                (match mzIdentMLXML.EnzymeName with
                 ...
                 |None -> Unchecked.defaultof<array<EnzymeNameParam>>
                )
    ...
    let convertToEntity_SpectrumIdentificationProtocol (dbContext:MzIdentMLContext) (mzIdentMLXML:SchemePeptideShaker.SpectrumIdentificationProtocol) =
        ...
        let spectrumIdentificationProtocolWithName = setOption SpectrumIdentificationProtocolHandler.addName spectrumIdentificationProtocolBasic mzIdentMLXML.Name
        let spectrumIdentificationProtocolWithAdditionalSearchParams = 
            SpectrumIdentificationProtocolHandler.addEnzymeAdditionalSearchParams
                ...
                (match mzIdentMLXML.AdditionalSearchParams with
                 ...
                 |None -> Unchecked.defaultof<array<AdditionalSearchParam>>
                )
        ...
        let spectrumIdentificationProtocolWithFragmentTolerances =
            SpectrumIdentificationProtocolHandler.addFragmentTolerances
                ...
                (match mzIdentMLXML.FragmentTolerance with
                 ...
                 |None -> Unchecked.defaultof<array<FragmentToleranceParam>>
                )
        let spectrumIdentificationProtocolWithParentTolerances =
            SpectrumIdentificationProtocolHandler.addParentTolerances
                ...
                (match mzIdentMLXML.ParentTolerance with
                 ...
                 |None -> Unchecked.defaultof<array<ParentToleranceParam>>
                )
        ...
        let spectrumIdentificationProtocolWithTranslationTable =
            SpectrumIdentificationProtocolHandler.addTranslationTables
                spectrumIdentificationProtocolWithDatabaseFrames
                (match mzIdentMLXML.DatabaseTranslation with
        ...
        let proteinDetectionProtocolWithAnalysisParams =
            ProteinDetectionProtocolHandler.addAnalysisParams
                ...
                (match mzIdentMLXML.AnalysisParams with 
                 ...
                 |None -> Unchecked.defaultof<array<AnalysisParam>>
                )
